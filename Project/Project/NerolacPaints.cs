...
namespace Project
{
    public partial class NerolacPaints : Form
    {
        SqlCommand cmd;
        SqlConnection cn;
        public NerolacPaints()
        {
            InitializeComponent();
        }
        ...
        private void Fillgrid()
        {
            ...
            string qry = "select *  from NerolacPaints";
        ...
        private void NerolacPaints_Load(object sender, EventArgs e)
        {
            Fillgrid();
        }
        ...
        private void btnSave_Click_1(object sender, EventArgs e)
        {
            if (validate() == true)
            {
                ...
                {
                    cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DataBaseConn"].ConnectionString);
                    string qry = "insert into NerolacPaints(Name,Quantity,Cost,Type,Durability,Assurance,Stock,Token)values(@Name,@Quantity,@Cost,@Type,@Durability,@Assurance,@Stock,@Token)";
                    cmd = new SqlCommand(qry, cn);
        ...
        private void btnUpdate_Click_1(object sender, EventArgs e)
        {
            if (validate() == true)
            {
                ...
                {
                    cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DataBaseConn"].ConnectionString);
                    string qry = "update NerolacPaints set Name=@Name,Quantity=@Quantity,Cost=@Cost,Type=@Type,Durability=@Durability,Assurance=@Assurance,Stock=@Stock,Token=@Token where Code=@Code";
                    cmd = new SqlCommand(qry, cn);
                    cmd.Parameters.AddWithValue("@Name", txtName.Text);
                    cmd.Parameters.AddWithValue("@Quantity", cmbQty.Text);
                    cmd.Parameters.AddWithValue("@Cost", txtCost.Text);
        ...
        private void btnDelete_Click_1(object sender, EventArgs e)
        {
            if (validate() == true)
            {
                ...
                {
                    cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DataBaseConn"].ConnectionString);
                    string qry = "delete from NerolacPaints where Code=@Code";
                    cmd = new SqlCommand(qry, cn);
                    cmd.Parameters.AddWithValue("@Code", lblCode.Text);
                    cn.Open();
                    cmd.ExecuteNonQuery();
        ...
        private void dgvNP_RowHeaderMouseClick_1(object sender, DataGridViewCellMouseEventArgs e)
        {
            lblCode.Text = dgvNP.Rows[e.RowIndex].Cells[0].Value.ToString();
            txtName.Text = dgvNP.Rows[e.RowIndex].Cells[1].Value.ToString();
            cmbQty.Text = dgvNP.Rows[e.RowIndex].Cells[2].Value.ToString();
            txtCost.Text = dgvNP.Rows[e.RowIndex].Cells[3].Value.ToString();
            cmbType.Text = dgvNP.Rows[e.RowIndex].Cells[4].Value.ToString();
            txtDur.Text = dgvNP.Rows[e.RowIndex].Cells[5].Value.ToString();
            string a = dgvNP.Rows[e.RowIndex].Cells[6].Value.ToString();
            if (a == "Yes")
                rdbYes.Checked = true;
            else if (a == "No")
                rdbNo.Checked = true;
            txtStock.Text = dgvNP.Rows[e.RowIndex].Cells[7].Value.ToString();
            string t = dgvNP.Rows[e.RowIndex].Cells[8].Value.ToString();
            if (t == "Available")
