...
namespace Project
{
    public partial class DuluxPaints : Form
    {
        SqlCommand cmd;
        SqlConnection cn;
        public DuluxPaints()
        ...
        private void DuluxPaint_Load(object sender, EventArgs e)
        {
            Fillgrid();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (validate() == true)
            {
                try
                {
                    ...
                    string qry = "insert into DuluxPaint(Name,Quantity,Cost,Type,Durability,Assurance,Stock,Token)values(@Name,@Quantity,@Cost,@Type,@Durability,@Assurance,@Stock,@Token)";
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (validate() == true)
            {
                ...
                {
                    cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DataBaseConn"].ConnectionString);
                    String qry = "update DuluxPaint set Name=@Name,Quantity=@Quantity,Cost=@Cost,Type=@Type,Durability=@Durability,Assurance=@Assurance,Stock=@Stock,Token=@Token where Code=@Code";
                    cmd = new SqlCommand(qry, cn);
                    cmd.Parameters.AddWithValue("@Name", txtName.Text);
                    cmd.Parameters.AddWithValue("@Quantity", cmbQty.Text);
                    cmd.Parameters.AddWithValue("@Cost", txtCost.Text);
        ...
        {
            cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DataBaseConn"].ConnectionString);
            string qry = "select *  from DuluxPaint";
            cmd = new SqlCommand(qry, cn);
        ...
        private void dgvDP_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            lblCode.Text = dgvDP.Rows[e.RowIndex].Cells[0].Value.ToString();
            txtName.Text = dgvDP.Rows[e.RowIndex].Cells[1].Value.ToString();
            cmbQty.Text = dgvDP.Rows[e.RowIndex].Cells[2].Value.ToString();
            txtCost.Text = dgvDP.Rows[e.RowIndex].Cells[3].Value.ToString();
            cmbType.Text = dgvDP.Rows[e.RowIndex].Cells[4].Value.ToString();
            txtDur.Text = dgvDP.Rows[e.RowIndex].Cells[5].Value.ToString();
            string a = dgvDP.Rows[e.RowIndex].Cells[6].Value.ToString();
            if (a == "Yes")
                rdbYes.Checked = true;
            else if (a == "No")
                rdbNo.Checked = true;
            txtStock.Text = dgvDP.Rows[e.RowIndex].Cells[7].Value.ToString();
            string t = dgvDP.Rows[e.RowIndex].Cells[8].Value.ToString();
            if (t == "Available")
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (validate() == true)
            {
                ...
                {
                    cn = new SqlConnection(ConfigurationManager.ConnectionStrings["DataBaseConn"].ConnectionString);
                    string qry = "delete from DuluxPaint where Code=@Code";
                    cmd = new SqlCommand(qry, cn);
                    cmd.Parameters.AddWithValue("@Code", lblCode.Text);
                    cn.Open();
                    cmd.ExecuteNonQuery();
        ...
        private void DuluxPaints_Load(object sender, EventArgs e)
        {
            Fillgrid();
        }
    }
